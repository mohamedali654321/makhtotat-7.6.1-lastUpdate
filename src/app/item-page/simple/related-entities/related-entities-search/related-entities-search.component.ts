...
export class RelatedEntitiesSearchComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.newConfiguration=this.configuration.split('-')[0];
    if(this.newConfiguration === 'orgunit'){
      this.filterFields='OrgUnit'
     }
     else if(this.newConfiguration === 'volume'){

      this.filterFields='JournalVolume'
     }
     else if(this.newConfiguration === 'default'){

      this.filterFields='Publication'
     }
     else{
      this.filterFields=this.capitalizeFirstLetter(this.newConfiguration)
     }
