...
export class DSOSelectorComponent implements OnInit, OnDestroy {
  ...
  search(query: string, page: number, useCache: boolean = true): Observable<RemoteData<PaginatedList<SearchResult<DSpaceObject>>>> {
   ...
   let isEntitySearch = document.URL.includes('source=browseCategory') && document.URL.includes('value');
  if(isEntitySearch){
   
    let efectiveSort = query ? null : this.sort;
    return this.searchService.search(
      new PaginatedSearchOptions({
        query:`${this.localeService.getStringByLocale(this.route.snapshot.queryParams.value)}`,
        dsoTypes: this.types,
